...
class _A2A(torch.autograd.Function):
    """
    the upstream of pytorch titan use `all_to_all_single_autograd`
    which on pytorch 2.6 can cause the memory leaking with activation checkpointing
    """

    @staticmethod
    def forward(ctx, x, out_splits, in_splits, group):
        # Ensure Python lists (CPU metadata only)
        ...
        # Sanity checks (catch mismatches early)
        assert x.dim() >= 1
        assert (
            sum(in_splits) == x.shape[0]
        ), "sum(input_split_sizes) must equal local input length"
...
class ExpertParallel(ParallelStyle):
    ...
    def _token_dispatch(self, mod, inputs, device_mesh):
        ...
        with torch.no_grad():
            ...
            dist.all_to_all_single(
                num_tokens_per_expert_group, num_tokens_per_expert, group=group
            )
            # NOTE: this would incur a device-to-host sync
            self.input_splits = (
                num_tokens_per_expert.view(ep_size, -1).sum(dim=1).tolist()
            )
            self.output_splits = (
                num_tokens_per_expert_group.view(ep_size, -1).sum(dim=1).tolist()
            )
        routed_input = a2a(routed_input, self.output_splits, self.input_splits, group)
    ...
    def _token_combine(self, mod, routed_output, device_mesh):
        # Reverse the direction (input/output splits swap)
        routed_output = a2a(
            routed_output,
            self.input_splits,
            self.output_splits,
...
def expert_parallel(func: Callable) -> Callable:
    ...
    def wrapper(
        ...
        num_ep_ranks = m_sizes.shape[0] // experts_per_ep_rank

        if num_ep_ranks == 1:
            return func(w1, w2, w3, act_fn, out_norm, x, m_sizes)
        ...
        out = func(w1, w2, w3, act_fn, out_norm, x, m_sizes)
