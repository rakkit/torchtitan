...
def parallelize_llama(
    ...
    if parallel_dims.tp_enabled:
        # with norm-everywhere, we actaully cannot use async TP (for Feed-Forward layer)
...
def apply_moe_ep_tp(
    ...
    2) We need to name the MoE layer as "moe" and dense layer as "feed_forward", we cannot use the same name "feed_forward" for both.
...
def apply_fsdp(
    ...
    for layer_id, transformer_block in model.layers.items():
        ...
        if transformer_block.moe_enabled and dp_mod_ep_mesh:
            ...
            # still need this, and upstream did not fixed it yet
