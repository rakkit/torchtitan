...
def generate_module_names_per_stage(
    ...
    if num_stages > num_effective_layers:
        raise ValueError(
            f"Number of stages ({num_stages}) cannot be greater than effective layers "
            f"({num_effective_layers})"
        )
    ...
    if layers_per_stage < max(input_weight, output_weight):
        raise ValueError(
            f"Layers per stage ({layers_per_stage}) must be >= max(input_weight={input_weight}, "
