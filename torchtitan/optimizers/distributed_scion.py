...
class DistributedScion(torch.optim.Optimizer):
    def __init__(
        ...
        experts_weights_layour="G-D_in-D_out",
    ):
        ...
        assert experts_weights_layour in [
            "G-D_in-D_out",
            "G-D_out-D_in",
        ], f"Unknown experts weights layout: {experts_weights_layour}"
        self.experts_need_transpose = experts_weights_layour == "G-D_in-D_out"
    ...
    def lmo(
        ...
        if g.ndim == 2:
            ...
            # TODO(JSC): JUST HARD CODE IT TO USE 'identity' backend and 'bias_rms' norm_factor for now
            # until we add regex to extra the norm's weights
