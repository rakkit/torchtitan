...
def quantize_fp8_rowwise(
    ...
    Currently this function differs from AQT's `fp8_weight_only()` in the following way:
...
class FP8QuantizedTrainingLinearWeight(TorchAOBaseTensor):
    """FP8 symmetric quantization weight, with absmax scaling [-127, 127]. The main difference
    ...
    2. All in-place ops, such as `aten.copy_`, will perform stochastic rounding.
        `FP8QTLinearWeight.from_float()` does not perform stochastic rounding.
    3. The numerics for quantization is slightly different. See `quantize_fp8_rowwise()`
        for more details.
    """
    ...
    def __init__(self, int_data: Tensor, scale: Tensor):
        """Create a symmetric quantized FP8 weight. This tensor will appear to have the same dtype
    ...
    def from_float(cls, tensor: Tensor):
        """Convert a float tensor into FP8 quantized weight. No stochastic rounding is performed.
        This function is not differentiable.
        """
        int_data, scale = quantize_fp8_rowwise(tensor.detach())
    ...
    def fsdp_post_all_gather(
        ...
        *,
        ...
        return FP8QuantizedTrainingLinearWeight(int_data, scale), all_gather_outputs


class _FP8WeightOnlyLinear(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        input: Tensor,
        weight: FP8QuantizedTrainingLinearWeight,
...
implements = FP8QuantizedTrainingLinearWeight.implements


@implements(torch.nn.functional.linear)
def _(func, types, args, kwargs):
    return _FP8WeightOnlyLinear.apply(*args, **kwargs)
...
def _(func, types, args, kwargs):
    ...
    out = FP8QuantizedTrainingLinearWeight(
...
def _(func, types, args, kwargs):
    args = [
        x.dequantize() if isinstance(x, FP8QuantizedTrainingLinearWeight) else x
        for x in args
    ]
...
def _(func, types, args, kwargs):
    if isinstance(args[0], FP8QuantizedTrainingLinearWeight) and isinstance(
        args[1], FP8QuantizedTrainingLinearWeight
    ):
        args[0].int_data.copy_(args[1].int_data, **kwargs)
        args[0].scale.copy_(args[1].scale, **kwargs)

    elif isinstance(args[0], FP8QuantizedTrainingLinearWeight):
        int_data, scale = quantize_fp8_rowwise(args[1], stochastic_rounding=True)
...
def _(func, types, args, kwargs):
    if len(args) == 3 and args[2] != 0:
        raise NotImplementedError("FP8QTLinearWeight only supports split at dim=0")

    fp8_weight: FP8QuantizedTrainingLinearWeight = args[0]
    int_data_list = func(fp8_weight.int_data, *args[1:], **kwargs)
    scale_list = func(fp8_weight.scale, *args[1:], **kwargs)

    out = [
        FP8QuantizedTrainingLinearWeight(int_data, scale)
...
def _(func, types, args, kwargs):
    ...
    return FP8QuantizedTrainingLinearWeight(int_data, scale)
...
def _(func, types, args, kwargs):
    out = FP8QuantizedTrainingLinearWeight(args[0].int_data, args[0].scale)
    return return_and_correct_aliasing(func, args, kwargs, out)


class FP8WeightOnlyQuantizedTrainingConfig(AOBaseConfig):
    pass


# for bc
fp8_weight_only_quantized_training = FP8WeightOnlyQuantizedTrainingConfig


@register_quantize_module_handler(FP8WeightOnlyQuantizedTrainingConfig)
def _fp8_weight_only_quantized_training_transform(
    module: torch.nn.Module,
    config: FP8WeightOnlyQuantizedTrainingConfig,
) -> torch.nn.Module:
    new_weight = FP8QuantizedTrainingLinearWeight.from_float(module.weight)
