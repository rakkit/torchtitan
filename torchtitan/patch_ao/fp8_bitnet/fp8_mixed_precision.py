...
@dataclass
class FP8MixedPrecisionTrainingConfig(AOBaseConfig):
    output: bool = True
...
# for bc
fp8_mixed_precision_training = FP8MixedPrecisionTrainingConfig


_DEFAULT_CONFIG = FP8MixedPrecisionTrainingConfig()


aten = torch.ops.aten


class FP8MixedPrecisionTrainingLinearWeight(TorchAOBaseTensor):
    """Linear weight for FP8 mixed-precision training. The weight is in original precision (e.g. FP32 or BF16).
    During training, weight and activation are dynamically quantized and cast to FP8 to utilize FP8 Tensor Cores,
...
def _dynamic_fp8_mm(A: Tensor, B: Tensor) -> Tensor:
    """Dynamically quantize A and B to perform FP8 matmul, then scale the results back to original precision.
    ...
    We transpose B before quantization for 2 reasons:
      - FP8 matmul is the most performant when A is row-major and B is column-major.
    ...
    `.contiguous()` to outputs of the quantize op to make sure:
      - Performant layout for FP8 matmul inputs (see above).
...
class _FP8MixedPrecisionTrainingLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if config.output:
            out = _dynamic_fp8_mm(input, weight.T)
        else:
            out = input @ weight.T
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            if ctx.config.grad_input:
                grad_input = _dynamic_fp8_mm(grad_output, weight)
            else:
        ...
        if ctx.needs_input_grad[1]:
            ...
            if ctx.config.grad_weight:
                # grad_weight = _dynamic_fp8_mm(grad_output.T, input)
                grad_weight = _dynamic_fp8_mm(
                    input.T, grad_output
...
@register_quantize_module_handler(FP8MixedPrecisionTrainingConfig)
def _fp8_mixed_precision_training_transform(
    ...
    if module_swap:
        ...
        module.__class__ = FP8MixedPrecisionTrainingLinear
        ...
        new_weight = FP8MixedPrecisionTrainingLinearWeight(module.weight, config)
