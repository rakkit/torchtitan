...
# LICENSE file in the root directory of this source tree.
# TODO: might merge this with torchao/kernel/fp8mm_triton.py
...
def _scaled_fp8_mm_kernel(
    ...
    if COL_SCALE_SCALAR:
        # hack to support FP8BitNet. col_scale is now a scalar
...
lib.define(
    "scaled_fp8_mm(Tensor A, Tensor B, Tensor A_scale, Tensor B_scale) -> Tensor"
)


def scaled_fp8_mm(A: Tensor, B: Tensor, row_scale: Tensor, col_scale: Tensor) -> Tensor:
    """Compute `(A @ B) * row_scale * col_scale`, where `A` and `B` are FP8 to utilize
    FP8 tensor cores. `col_scale` can be a scalar.
    """
    ...
    return torch.ops.torchao.scaled_fp8_mm(A, B, row_scale, col_scale)
...
@torch.library.impl(lib, "scaled_fp8_mm", "CUDA")
def scaled_fp8_mm_cuda(A: Tensor, B: Tensor, row_scale: Tensor, col_scale: Tensor):
    M, K = A.shape
    _, N = B.shape
