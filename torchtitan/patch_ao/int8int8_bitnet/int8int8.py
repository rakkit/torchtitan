...
class INT8INT8BitNetQuantizedTrainingLinearWeight(TorchAOBaseTensor):
    ...
    def fsdp_post_all_gather(
        ...
        *,
        ...
        return (
            INT8INT8BitNetQuantizedTrainingLinearWeight(int_data, scale),
            all_gather_outputs,
        )


class _INT8INT8BitNetWeightOnlyLinear(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        input: Tensor,
        weight: INT8INT8BitNetQuantizedTrainingLinearWeight,
...
implements = INT8INT8BitNetQuantizedTrainingLinearWeight.implements


@implements(torch.nn.functional.linear)
def _(func, types, args, kwargs):
    return _INT8INT8BitNetWeightOnlyLinear.apply(*args, **kwargs)
...
def _(func, types, args, kwargs):
    ...
    out = INT8INT8BitNetQuantizedTrainingLinearWeight(
...
def _(func, types, args, kwargs):
    args = [
        (
            x.dequantize()
            if isinstance(x, INT8INT8BitNetQuantizedTrainingLinearWeight)
            else x
        )
        for x in args
    ]
...
def _(func, types, args, kwargs):
    if isinstance(args[0], INT8INT8BitNetQuantizedTrainingLinearWeight) and isinstance(
        args[1], INT8INT8BitNetQuantizedTrainingLinearWeight
    ):
        args[0].int_data.copy_(args[1].int_data, **kwargs)
        args[0].scale.copy_(args[1].scale, **kwargs)

    elif isinstance(args[0], INT8INT8BitNetQuantizedTrainingLinearWeight):
...
def _(func, types, args, kwargs):
    ...
    int8_weight: INT8INT8BitNetQuantizedTrainingLinearWeight = args[0]
...
def _(func, types, args, kwargs):
    ...
    return INT8INT8BitNetQuantizedTrainingLinearWeight(int_data, scale)
...
def _(func, types, args, kwargs):
    out = INT8INT8BitNetQuantizedTrainingLinearWeight(args[0].int_data, args[0].scale)
    return return_and_correct_aliasing(func, args, kwargs, out)


class INT8INT8BitNetWeightOnlyQuantizedTrainingConfig(AOBaseConfig):
    pass


# for bc
int8int8_bitnet_weight_only_quantized_training = (
    INT8INT8BitNetWeightOnlyQuantizedTrainingConfig
)


@register_quantize_module_handler(INT8INT8BitNetWeightOnlyQuantizedTrainingConfig)
def _int8_weight_only_quantized_training_transform(
    module: torch.nn.Module,
    config: INT8INT8BitNetWeightOnlyQuantizedTrainingConfig,
) -> torch.nn.Module:
    new_weight = INT8INT8BitNetQuantizedTrainingLinearWeight.from_float(module.weight)
