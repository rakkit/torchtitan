...
@dataclass
class INT8INT8BitNetMixedPrecisionTrainingConfig(AOBaseConfig):
    output: bool = True
...
# for bc
int8int8_bitnet_mixed_precision_training = INT8INT8BitNetMixedPrecisionTrainingConfig


_DEFAULT_CONFIG = INT8INT8BitNetMixedPrecisionTrainingConfig()


aten = torch.ops.aten


class INT8INT8BitNetMixedPrecisionTrainingLinearWeight(TorchAOBaseTensor):
    ...
    def __new__(cls, data: Tensor, config: INT8INT8BitNetMixedPrecisionTrainingConfig):
    ...
    @torch._dynamo.disable
    def __init__(
        self, data: Tensor, config: INT8INT8BitNetMixedPrecisionTrainingConfig
    ):
        self._data = data
        self.config = config
    ...
    def fsdp_post_all_gather(
        ...
        *,
        ...
        (config,) = metadata
        if out is not None:
            assert isinstance(out, INT8INT8BitNetMixedPrecisionTrainingLinearWeight)
            assert out.config == config
            return
        return (
            INT8INT8BitNetMixedPrecisionTrainingLinearWeight(data, config),
            all_gather_outputs,
        )


@INT8INT8BitNetMixedPrecisionTrainingLinearWeight.implements(torch.nn.functional.linear)
def _(func, types, args, kwargs):
    if torch.is_autocast_enabled("cuda"):
        dtype = torch.get_autocast_gpu_dtype()
        args = tuple(x.to(dtype) if x is not None else x for x in args)
    return _INT8INT8BitNetMixedPrecisionTrainingLinearFunction.apply(*args, **kwargs)


class INT8INT8BitNetMixedPrecisionTrainingLinear(nn.Linear):
    def __init__(
        self, *args, config: INT8INT8BitNetMixedPrecisionTrainingConfig, **kwargs
    ) -> None:
        super().__init__(*args, **kwargs)
        self.config = config

    def forward(self, input: Tensor) -> Tensor:
        return _INT8INT8BitNetMixedPrecisionTrainingLinearFunction.apply(
...
class _INT8INT8BitNetMixedPrecisionTrainingLinearFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        input: Tensor,
        weight: Union[INT8INT8BitNetMixedPrecisionTrainingLinearWeight, Tensor],
        bias: Optional[Tensor],
        config: Optional[INT8INT8BitNetMixedPrecisionTrainingConfig] = None,
    ):
        ...
        if isinstance(weight, INT8INT8BitNetMixedPrecisionTrainingLinearWeight):
...
@register_quantize_module_handler(INT8INT8BitNetMixedPrecisionTrainingConfig)
def _int8_mixed_precision_training_transform(
    module: torch.nn.Module,
    config: INT8INT8BitNetMixedPrecisionTrainingConfig,
):
    ...
    if module_swap:
        ...
        module.__class__ = INT8INT8BitNetMixedPrecisionTrainingLinear
        ...
        new_weight = INT8INT8BitNetMixedPrecisionTrainingLinearWeight(
